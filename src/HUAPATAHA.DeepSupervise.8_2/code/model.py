    ...
    def build_model(self):
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = .4 * tf.reduce_mean(losses) \
                        + 0.15*(.8 * self.u_loss + .2 * self.u_sen_losses) \
                        + 0.15*(.8 * self.p_loss + .2 * self.p_sen_losses) \
                        + 0.15*(.8 * self.h_loss + .2 * self.h_sen_losses) \
                        + 0.15*(.8 * self.t_loss + .2 * self.t_sen_losses)
