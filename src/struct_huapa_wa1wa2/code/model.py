...
class HUAPA(object):
    ...
    def __init__(self, max_sen_len, max_doc_len, class_num, embedding_file,
        ...
        self.max_sen_len2 = max_sen_len * 2
        ...
        with tf.name_scope('weights'):
            self.weights = {
                ...
                'u_wh_3': tf.Variable(tf.random_uniform([2 * self.hidden_size, 2 * self.hidden_size], -0.01, 0.01)),
                'u_v_3': tf.Variable(tf.random_uniform([2 * self.hidden_size, 1], -0.01, 0.01)),
                ...
                'p_wh_3': tf.Variable(tf.random_uniform([2 * self.hidden_size, 2 * self.hidden_size], -0.01, 0.01)),
                'p_v_3': tf.Variable(tf.random_uniform([2 * self.hidden_size, 1], -0.01, 0.01)),
                ...
                'wu_3': tf.Variable(tf.random_uniform([2 * self.hidden_size, 2 * self.hidden_size], -0.01, 0.01)),
                'wp_3': tf.Variable(tf.random_uniform([2 * self.hidden_size, 2 * self.hidden_size], -0.01, 0.01)),
            }
        ...
        with tf.name_scope('biases'):
            self.biases = {
                ...
                'u_wh_3': tf.Variable(tf.random_uniform([2 * self.hidden_size], -0.01, 0.01)),
                ...
                'p_wh_3': tf.Variable(tf.random_uniform([2 * self.hidden_size], -0.01, 0.01)),
            }
    ...
    def user_attention(self):
        ...
        outputs = tf.reshape(
            tf.tile(outputs, [1, 1, self.max_sen_len]),
            [-1, self.max_sen_len, 2*self.hidden_size]
        )
        inputs2 = tf.concat([outputs, inputs], 2)
        ...
        batch_size = tf.shape(outputs)[0]
        with tf.name_scope('u_word_attention2'):
            output = tf.reshape(outputs, [-1, 2 * self.hidden_size])
            u = tf.matmul(output, self.weights['u_wh_3']) + self.biases['u_wh_3']
            u = tf.reshape(u, [-1, self.max_doc_len*self.max_sen_len, 2*self.hidden_size])
            u += tf.matmul(self.user, self.weights['wu_3'])[:,None,:]
            u = tf.tanh(u)
            u = tf.reshape(u, [-1, 2 * self.hidden_size])
            alpha = tf.reshape(tf.matmul(u, self.weights['u_v_3']),
                               [batch_size, 1, self.max_sen_len])
            alpha = self.softmax(alpha, self.sen_len, self.max_sen_len)
            outputs = tf.matmul(alpha, outputs)
    ...
    def product_attention(self):
        ...
        with tf.name_scope('p_word_encode2'):
            ...
            outputs = tf.concat(outputs, 2)
        batch_size = tf.shape(outputs)[0]
        with tf.name_scope('p_word_attention2'):
            output = tf.reshape(outputs, [-1, 2 * self.hidden_size])
            u = tf.matmul(output, self.weights['p_wh_3']) + self.biases['p_wh_3']
            u = tf.reshape(u, [-1, self.max_doc_len*self.max_sen_len, 2*self.hidden_size])
            u += tf.matmul(self.user, self.weights['wp_3'])[:,None,:]
            u = tf.tanh(u)
            u = tf.reshape(u, [-1, 2 * self.hidden_size])
            alpha = tf.reshape(tf.matmul(u, self.weights['p_v_3']),
                               [batch_size, 1, self.max_sen_len])
            alpha = self.softmax(alpha, self.sen_len, self.max_sen_len)
            outputs = tf.matmul(alpha, outputs)
        outputs = tf.reshape(outputs, [-1, self.max_doc_len, 2 * self.hidden_size])
    ...
    def build_model(self):
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = 0.4*tf.reduce_mean(losses) \
                        + 0.3*(.7*self.u_loss + .3*self.u_sen_losses) \
                        + 0.3*(.7*self.p_loss + .3*self.p_sen_losses)
