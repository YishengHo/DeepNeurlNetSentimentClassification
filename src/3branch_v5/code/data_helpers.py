...
class Dataset(object):
    def __init__(self, data_file):
        ...
        with open(data_file, 'r') as f:
            ...
            for line in f:
                ...
                d = line[3].strip().lower()
                self.t_docs.append(d)
                self.t_tme.append(line[4].strip().lower())
                self.t_hlp.append(line[6].strip().lower())
                s = line[8].strip().lower()

                self.t_sums.append(s)
                self.t_sd.append(s + d)
    ...
    def genBatch(self, usrdict, prddict, hlpdict, tmedict, wordsdict, batch_size, max_sen_len, max_doc_len, n_class):
        ...
        for i in range(self.epoch):
            ...
            b_sums, b_slen_s, b_dlen_s = fit_transform(self.t_sums[i*batch_size:(i+1)*batch_size],
                                                          wordsdict, max_sen_len, max_doc_len)
            b_sd, b_slen_sd, b_dlen_sd = fit_transform(self.t_sd[i*batch_size:(i+1)*batch_size],
                                                          wordsdict, max_sen_len, max_doc_len)
            self.docs.append(b_docs)
            self.slen_d.append(b_sen_len)
            self.dlen_d.append(b_doc_len)

            self.sums.append(b_sums)
            self.slen_s.append(b_slen_s)
            self.dlen_s.append(b_dlen_s)

            self.sumdocs.append(b_sd)
            self.slen_sd.append(b_slen_sd)
            self.dlen_sd.append(b_dlen_sd)
    ...
    def batch_iter(self, usrdict, prddict, hlpdict, tmedict, wordsdict, n_class, batch_size, num_epochs, max_sen_len, max_doc_len, shuffle=True):
        ...
        self.t_sums = np.asarray(self.t_sums)
        self.t_sd = np.asarray(self.t_sd)
        ...
        for epoch in range(num_epochs):
            ...
            if shuffle:
                ...
                self.t_sums = self.t_sums[shuffle_indices]
                self.t_sd = self.t_sd[shuffle_indices]
            ...
            for batch_num in range(num_batches_per_epoch):
                ...
                docs, slen_d, dlen_d = fit_transform(self.t_docs[start:end], wordsdict, max_sen_len, max_doc_len)
                sums, slen_s, dlen_s = fit_transform(self.t_sums[start:end], wordsdict, max_sen_len, max_doc_len)
                sd, slen_sd, dlen_sd = fit_transform(self.t_sd[start:end], wordsdict, max_sen_len, max_doc_len)
                batch_data = zip(usr, prd, hlp, tme, docs, label, slen_d, dlen_d, sums, slen_s, dlen_s, sd, slen_sd, dlen_sd)
                yield batch_data
