...
class huapahata(object):
    ...
    def build_model(self):
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = 0.7 * tf.reduce_mean(losses) \
                        + 0.5 * (0.4*tf.reduce_mean(losses_d) + 0.6*(0.25*self.d_u_loss  + 0.25*self.d_p_loss + 0.25*self.d_h_loss + 0.25*self.d_t_loss)) \
                        + 0.5 * (0.4*tf.reduce_mean(losses_s) + 0.6*(0.25*self.s_u_loss  + 0.25*self.s_p_loss + 0.25*self.s_h_loss + 0.25*self.s_t_loss)) \
                        + 0.2 * (0.4*tf.reduce_mean(losses_sd) + 0.6*(0.25*self.sd_u_loss  + 0.25*self.sd_p_loss + 0.25*self.sd_h_loss + 0.25*self.sd_t_loss))
