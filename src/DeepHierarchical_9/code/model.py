...
class huapahata(object):
    ...
    def __init__(self, max_sen_len, max_doc_len, class_num, embedding_file,
        ...
        with tf.name_scope('weights'):
            self.weights = {
                ...
                'h_sen_softmax': tf.Variable(tf.random_uniform([ self.max_doc_len * self.hidden_size, self.class_num], -0.01, 0.01)),
                'h_softmax': tf.Variable(tf.random_uniform([ self.hidden_size, self.class_num], -0.01, 0.01)),
                'h_wh_1': tf.Variable(tf.random_uniform([ self.hidden_size,  self.hidden_size], -0.01, 0.01)),
                'h_v_1': tf.Variable(tf.random_uniform([ self.hidden_size, 1], -0.01, 0.01)),
                'h_wh_2': tf.Variable(tf.random_uniform([ self.hidden_size,  self.hidden_size], -0.01, 0.01)),
                'h_v_2': tf.Variable(tf.random_uniform([ self.hidden_size, 1], -0.01, 0.01)),

                't_sen_softmax': tf.Variable(tf.random_uniform([ self.max_doc_len * self.hidden_size, self.class_num], -0.01, 0.01)),
                't_softmax': tf.Variable(tf.random_uniform([ self.hidden_size, self.class_num], -0.01, 0.01)),
                't_wh_1': tf.Variable(tf.random_uniform([ self.hidden_size,  self.hidden_size], -0.01, 0.01)),
                't_v_1': tf.Variable(tf.random_uniform([ self.hidden_size, 1], -0.01, 0.01)),
                't_wh_2': tf.Variable(tf.random_uniform([ self.hidden_size,  self.hidden_size], -0.01, 0.01)),
                't_v_2': tf.Variable(tf.random_uniform([ self.hidden_size, 1], -0.01, 0.01)),
                ...
                'wh_1': tf.Variable(tf.random_uniform([ self.hidden_size,  self.hidden_size], -0.01, 0.01)),
                'wt_1': tf.Variable(tf.random_uniform([ self.hidden_size,  self.hidden_size], -0.01, 0.01)),
                ...
                'wh_2': tf.Variable(tf.random_uniform([ self.hidden_size,  self.hidden_size], -0.01, 0.01)),
                'wt_2': tf.Variable(tf.random_uniform([ self.hidden_size,  self.hidden_size], -0.01, 0.01)),
            }
        ...
        with tf.name_scope('biases'):
            self.biases = {
                ...
                'h_sen_softmax': tf.Variable(tf.random_uniform([self.class_num], -0.01, 0.01)),
                'h_softmax': tf.Variable(tf.random_uniform([self.class_num], -0.01, 0.01)),
                'h_wh_1': tf.Variable(tf.random_uniform([ self.hidden_size], -0.01, 0.01)),
                'h_wh_2': tf.Variable(tf.random_uniform([ self.hidden_size], -0.01, 0.01)),

                't_sen_softmax': tf.Variable(tf.random_uniform([self.class_num], -0.01, 0.01)),
                't_softmax': tf.Variable(tf.random_uniform([self.class_num], -0.01, 0.01)),
                't_wh_1': tf.Variable(tf.random_uniform([ self.hidden_size], -0.01, 0.01)),
                't_wh_2': tf.Variable(tf.random_uniform([ self.hidden_size], -0.01, 0.01)),
        ...
        with tf.name_scope('embedding'):
            ...
            self.helpfulness_embedding = tf.Variable(tf.random_uniform([self.helpfulness_num, self.hidden_size], -0.01, 0.01), dtype=tf.float32)
            self.time_embedding = tf.Variable(tf.random_uniform([self.time_num, self.hidden_size], -0.01, 0.01), dtype=tf.float32)
    ...
    def helpfulness_attention(self):
        ...
        inputs2 = tf.reshape(outputs1, [-1, self.max_sen_len, self.hidden_size])
        with tf.name_scope('h_word_encode2'):
            outputs2, state = tf.nn.dynamic_rnn(
                cell=tf.nn.rnn_cell.LSTMCell(self.hidden_size, forget_bias=1.0),
                inputs=inputs2,
                sequence_length=sen_len,
        ...
        inputs3 = tf.add(tf.reshape(outputs2, [-1, self.max_sen_len, self.hidden_size]), inputs2)
        with tf.name_scope('h_word_encode3'):
            outputs, state = tf.nn.dynamic_rnn(
                cell=tf.nn.rnn_cell.LSTMCell(self.hidden_size, forget_bias=1.0),
                inputs=inputs3,
                sequence_length=sen_len,
        ...
        # sentences deep supervise learning.
        with tf.name_scope('h_sen_softmax'):
            self.h_sens = tf.reshape(outputs, [-1,  self.max_doc_len * self.hidden_size]);
            self.h_sens_scores = tf.matmul(self.h_sens, self.weights['h_sen_softmax']) + self.biases['h_sen_softmax']
            # self.h_sens_predictions = tf.argmax(self.h_sens_scores, 1, name="h_sen_predictions")

        with tf.name_scope('h_sen_loss'):
            sen_losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.h_sens_scores, labels=self.input_y)
            self.h_sen_losses = tf.reduce_mean(sen_losses)
        ...
        inputs2 = tf.add(tf.reshape(outputs1, [-1, self.max_doc_len, self.hidden_size]), outputs)
        with tf.name_scope('h_sentence_encode2'):
            outputs, state = tf.nn.dynamic_rnn(
                cell=tf.nn.rnn_cell.LSTMCell(self.hidden_size, forget_bias=1.0),
                inputs=inputs2,
                sequence_length=self.doc_len,
        ...
        with tf.name_scope('h_softmax'):
            self.h_doc = tf.reshape(outputs, [batch_size,  self.hidden_size])
    ...
    def time_attention(self):
        ...
        inputs2 = tf.reshape(outputs1, [-1, self.max_sen_len, self.hidden_size])
        with tf.name_scope('t_word_encode2'):
            outputs2, state = tf.nn.dynamic_rnn(
                cell=tf.nn.rnn_cell.LSTMCell(self.hidden_size, forget_bias=1.0),
                inputs=inputs2,
                sequence_length=sen_len,
        ...
        inputs3 = tf.add(tf.reshape(outputs2, [-1, self.max_sen_len, self.hidden_size]), inputs2)
        with tf.name_scope('t_word_encode3'):
            outputs, state = tf.nn.dynamic_rnn(
                cell=tf.nn.rnn_cell.LSTMCell(self.hidden_size, forget_bias=1.0),
                inputs=inputs3,
                sequence_length=sen_len,
        ...
        # sentences deep supervise learning.
        with tf.name_scope('t_sen_softmax'):
            self.t_sens = tf.reshape(outputs, [-1,  self.max_doc_len * self.hidden_size]);
            self.t_sens_scores = tf.matmul(self.t_sens, self.weights['t_sen_softmax']) + self.biases['t_sen_softmax']
            # self.t_sens_predictions = tf.argmax(self.t_sens_scores, 1, name="t_sen_predictions")

        with tf.name_scope('t_sen_loss'):
            sen_losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.t_sens_scores, labels=self.input_y)
            self.t_sen_losses = tf.reduce_mean(sen_losses)
        ...
        inputs2 = tf.add(tf.reshape(outputs1, [-1, self.max_doc_len, self.hidden_size]), outputs)
        with tf.name_scope('t_sentence_encode2'):
            outputs, state = tf.nn.dynamic_rnn(
                cell=tf.nn.rnn_cell.LSTMCell(self.hidden_size, forget_bias=1.0),
                inputs=inputs2,
                sequence_length=self.doc_len,
        ...
        with tf.name_scope('t_softmax'):
            self.t_doc = tf.reshape(outputs, [batch_size,  self.hidden_size])
    ...
    def build_model(self):
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = .5 * tf.reduce_mean(losses) \
                        + 0.125*(.8 * self.u_loss + .2 * self.u_sen_losses) \
                        + 0.125*(.8 * self.p_loss + .2 * self.p_sen_losses) \
                        + 0.125*(.8 * self.h_loss + .2 * self.h_sen_losses) \
                        + 0.125*(.8 * self.t_loss + .2 * self.t_sen_losses)
