...
class huapahata(object):
    ...
    def __init__(self, max_sen_len, max_doc_len, class_num, embedding_file,
        ...
        with tf.name_scope('weights'):
            self.weights = {
                ...
                'u_sen_softmax': tf.Variable(tf.random_uniform([2 * self.max_doc_len * self.hidden_size, self.class_num], -0.01, 0.01)),
                ...
                'p_sen_softmax': tf.Variable(tf.random_uniform([2 * self.max_doc_len * self.hidden_size, self.class_num], -0.01, 0.01)),
                ...
                'h_sen_softmax': tf.Variable(tf.random_uniform([2 * self.max_doc_len * self.hidden_size, self.class_num], -0.01, 0.01)),
                ...
                't_sen_softmax': tf.Variable(tf.random_uniform([2 * self.max_doc_len * self.hidden_size, self.class_num], -0.01, 0.01)),
    ...
    def user_attention(self):
        ...
        inputs2 = tf.reshape(outputs1, [-1, self.max_sen_len, self.hidden_size*2])
        with tf.name_scope('u_word_encode2'):
            ...
            outputs2 = tf.concat(outputs, 2)

        inputs3 = tf.add(tf.reshape(outputs2, [-1, self.max_sen_len, self.hidden_size*2]), inputs2)
        ...
        with tf.name_scope('u_sen_softmax'):
            self.u_sens = tf.reshape(outputs, [-1, 2 * self.max_doc_len * self.hidden_size]);
    ...
    def product_attention(self):
        ...
        with tf.name_scope('p_sen_softmax'):
            self.p_sens = tf.reshape(outputs, [-1, 2 * self.max_doc_len * self.hidden_size]);
    ...
    def helpfulness_attention(self):
        ...
        with tf.name_scope('h_sen_softmax'):
            self.h_sens = tf.reshape(outputs, [-1, 2 * self.max_doc_len * self.hidden_size]);
    ...
    def time_attention(self):
        ...
        with tf.name_scope('t_sen_softmax'):
            self.t_sens = tf.reshape(outputs, [-1, 2 * self.max_doc_len * self.hidden_size]);
    ...
    def build_model(self):
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = .3 * tf.reduce_mean(losses) \
                        + 0.175*(.7 * self.u_loss + .3 * self.u_sen_losses) \
                        + 0.175*(.7 * self.p_loss + .3 * self.p_sen_losses) \
                        + 0.175*(.7 * self.h_loss + .3 * self.h_sen_losses) \
                        + 0.175*(.7 * self.t_loss + .3 * self.t_sen_losses)
