...
# Training parameters
tf.flags.DEFINE_integer("batch_size", 40, "Batch Size")
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def train_step(batch):
            u, p, h, t, x, y, slen_d, dlen_d, x_s, slen_s, dlen_s, x_sd, slen_sd, dlen_sd = zip(*batch)
            feed_dict = {
                huapahata.userid: u,
                ...
                huapahata.timeid: t,
                huapahata.input_x: x,
                huapahata.input_x_s: x_s,
                huapahata.input_x_sd: x_sd,
                huapahata.input_y: y,
                huapahata.sen_len: slen_d,
                huapahata.doc_len: dlen_d
            }
        ...
        def predict_step(u, p, h, t, x, x_s, x_sd, y, sen_len, doc_len, name=None):
            feed_dict = {
                huapahata.userid: u,
                huapahata.productid: p,
                huapahata.helpfulnessid: h,
        ...
        def predict(dataset, name=None):
            ...
            for i in range(dataset.epoch):
                correct_num, _, mse = predict_step(dataset.usr[i], dataset.prd[i], dataset.hlp[i], dataset.tme[i],
                                                   dataset.docs[i], dataset.sums[i], dataset.sumdocs[i],
                                                   dataset.label[i], dataset.slen_sd[i], dataset.dlen_sd[i], name)
                acc += correct_num
                rmse += mse
