...
class huapahata(object):
    ...
    def user_attention(self):
        ...
        rnn_inputs = tf.nn.dropout(inputs2, keep_prob=.5)
        ...
        inputs3 = tf.add(tf.reshape(outputs2, [-1, self.max_sen_len, self.hidden_size*2]), outputs1)
        rnn_inputs = tf.nn.dropout(inputs3, keep_prob=.5)
        ...
        outputs = tf.add(tf.reshape(outputs, [-1, self.max_sen_len, self.hidden_size*2]), outputs1)
