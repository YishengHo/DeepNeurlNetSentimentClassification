    ...
    def build_model(self):
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = .4 * tf.reduce_mean(losses) \
                        + 0.15*(.5 * self.u_loss + .5 * self.u_sen_losses) \
                        + 0.15*(.5 * self.p_loss + .5 * self.p_sen_losses) \
                        + 0.15*(.5 * self.h_loss + .5 * self.h_sen_losses) \
                        + 0.15*(.5 * self.t_loss + .5 * self.t_sen_losses)
