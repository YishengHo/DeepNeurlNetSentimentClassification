...
class HUAPA(object):
    ...
    def __init__(self, max_sen_len, max_doc_len, class_num, embedding_file,
        ...
        self.max_doc_len2 = max_doc_len * 2
    ...
    def user_attention(self):
        ...
        outputs = tf.reshape(outputs, [batch_size, 2*self.hidden_size])
        outputs = tf.tile(outputs, [1, self.max_doc_len])
        inputs2 = tf.concat([outputs,
                             tf.reshape(sinputs, [batch_size, 2*self.max_doc_len*self.hidden_size])], 1)
        inputs2 = tf.reshape(inputs2, [-1, self.max_doc_len2, 2*self.hidden_size])
        doc_len = self.doc_len
        doc_len = tf.math.add(
            doc_len,
            tf.math.scalar_mul(self.max_doc_len, tf.ones([tf.size(doc_len)], tf.int32))
        )
        ...
        batch_size = tf.shape(outputs)[0]
        with tf.name_scope('u_sentence_attention2'):
            output = tf.reshape(outputs, [-1, 2 * self.hidden_size])
            u = tf.matmul(output, self.weights['u_wh_2']) + self.biases['u_wh_2']
            u = tf.reshape(u, [-1, self.max_doc_len2, 2*self.hidden_size])
            u += tf.matmul(self.user, self.weights['wu_2'])[:,None,:]
            u = tf.tanh(u)
            u = tf.reshape(u, [-1, 2*self.hidden_size])
            alpha = tf.reshape(tf.matmul(u, self.weights['u_v_2']),
                               [batch_size, 1, self.max_doc_len2])
            alpha = self.softmax(alpha, doc_len, self.max_doc_len2)
            outputs = tf.matmul(alpha, outputs)
    ...
    def product_attention(self):
        ...
        outputs = tf.reshape(outputs, [-1, self.max_doc_len, 2*self.hidden_size])
        outputs = tf.tile(outputs, [1, 1, self.max_sen_len])
        inputs2 = tf.concat([outputs,
                             tf.reshape(inputs, [-1, self.max_doc_len, self.max_sen_len*self.embedding_dim])], 2)
        inputs2 = tf.reshape(inputs2, [-1, self.max_sen_len2, self.embedding_dim])
        sen_len = tf.math.add(
            sen_len,
            tf.math.scalar_mul(self.max_sen_len, tf.ones([tf.size(sen_len)], tf.int32))
        )
        ...
        with tf.name_scope('p_word_attention2'):
            output = tf.reshape(outputs, [-1, 2 * self.hidden_size])
            u = tf.matmul(output, self.weights['p_wh_1']) + self.biases['p_wh_1']
            u = tf.reshape(u, [-1, self.max_doc_len*self.max_sen_len2, 2*self.hidden_size])
            u += tf.matmul(self.user, self.weights['wp_1'])[:,None,:]
            u = tf.tanh(u)
            u = tf.reshape(u, [-1, 2 * self.hidden_size])
            alpha = tf.reshape(tf.matmul(u, self.weights['p_v_1']),
                               [batch_size, 1, self.max_sen_len2])
            alpha = self.softmax(alpha, self.sen_len, self.max_sen_len2)
            outputs = tf.matmul(alpha, outputs)
        ...
        batch_size = tf.shape(outputs)[0]
        with tf.name_scope('p_sentence_attention2'):
            output = tf.reshape(outputs, [-1, 2 * self.hidden_size])
            u = tf.matmul(output, self.weights['p_wh_2']) + self.biases['p_wh_2']
            u = tf.reshape(u, [-1, self.max_doc_len2, 2*self.hidden_size])
            u += tf.matmul(self.user, self.weights['wp_2'])[:,None,:]
            u = tf.tanh(u)
            u = tf.reshape(u, [-1, 2*self.hidden_size])
            alpha = tf.reshape(tf.matmul(u, self.weights['p_v_2']),
                               [batch_size, 1, self.max_doc_len2])
