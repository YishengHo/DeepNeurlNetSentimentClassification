...
parser.add_argument("--embedding_dim", default=256, type=int, help="Dimensionality of character embedding")
parser.add_argument("--hidden_size", default=64, type=int, help="hidden_size of rnn")
parser.add_argument('--max_sen_len', default=40, type=int, help='max number of tokens per sentence')
parser.add_argument('--max_doc_len', default=30, type=int, help='max number of tokens per sentence')
parser.add_argument("--lr", default=0.001, type=float, help="Learning rate")

# Training parameters
parser.add_argument("--batch_size", default=100, type=int, help="Batch Size")
parser.add_argument("--num_epochs", default=30, type=int, help="Number of training epochs")
parser.add_argument("--evaluate_every", default=200, type=int,  help="Evaluate model on dev set after this many steps")
...
embeddingpath = '../../data/'+FLAGS.dataset+'/embedding_ebdsize256.txt'
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        huapataha.build_model()
        # Define Training procedure
        global_step = tf.Variable(0, name="global_step", trainable=False)
        optimizer = tf.train.AdamOptimizer(FLAGS.lr)
        grads_and_vars = optimizer.compute_gradients(huapataha.loss)
        ...
        def train_step(batch):
            ...
            feed_dict = {
                huapataha.userid: u,
                huapataha.productid: p,
                huapataha.input_x: x,
                huapataha.input_y: y,
                huapataha.sen_len: sen_len,
                huapataha.doc_len: doc_len
            }
            _, step, loss, accuracy = sess.run(
                [train_op, global_step, huapataha.loss, huapataha.accuracy],
                feed_dict)
        ...
        def predict_step(u, p, x, y, sen_len, doc_len, name=None):
            ...
            step, loss, accuracy, correct_num, mse = sess.run(
                [global_step, huapataha.loss, huapataha.accuracy, huapataha.correct_num, huapataha.mse],
                feed_dict)
        ...
        with open(FLAGS.dataset+"_"+"topacc.txt", 'a') as of:
            print("\npara:", FLAGS, file=of)
        for tr_batch in trainbatches:
            ...
            if current_step % FLAGS.evaluate_every == 0:
                ...
                if dev_acc >= better_dev_acc:
                    ...
                    with open(FLAGS.dataset+"_"+ str(FLAGS.lr)+"_"+str(FLAGS.evaluate_every)+"_"+"topacc.txt", 'a') as of:
                        print("\nEvaluation round %d:" % (predict_round), file=of)
                        print("topacc: %.4f   RMSE: %.4f    mdlID: {}\n".format(path) % (topacc, toprmse), file=of)
