...
class huapahata(object):
    ...
    def __init__(self, max_sen_len, max_doc_len, class_num, embedding_file,
        ...
        with tf.name_scope('weights'):
            self.weights = {
                'softmax': tf.Variable(tf.random_uniform([14 * self.hidden_size, self.class_num], -0.01, 0.01)),
                ...
                'up_softmax': tf.Variable(tf.random_uniform([2 * self.hidden_size, self.class_num], -0.01, 0.01)),
                'up_wh_1': tf.Variable(tf.random_uniform([2 * self.hidden_size, 2 * self.hidden_size], -0.01, 0.01)),
                'up_v_1': tf.Variable(tf.random_uniform([2 * self.hidden_size, 1], -0.01, 0.01)),
                'up_wh_2': tf.Variable(tf.random_uniform([2 * self.hidden_size, 2 * self.hidden_size], -0.01, 0.01)),
                'up_v_2': tf.Variable(tf.random_uniform([2 * self.hidden_size, 1], -0.01, 0.01)),
                ...
                'wupu_1': tf.Variable(tf.random_uniform([2 * self.hidden_size, 2 * self.hidden_size], -0.01, 0.01)),
                'wupp_1': tf.Variable(tf.random_uniform([2 * self.hidden_size, 2 * self.hidden_size], -0.01, 0.01)),
                ...
                'wupu_2': tf.Variable(tf.random_uniform([2 * self.hidden_size, 2 * self.hidden_size], -0.01, 0.01)),
                'wupp_2': tf.Variable(tf.random_uniform([2 * self.hidden_size, 2 * self.hidden_size], -0.01, 0.01)),
            }
        ...
        with tf.name_scope('biases'):
            self.biases = {
                ...
                'up_softmax': tf.Variable(tf.random_uniform([self.class_num], -0.01, 0.01)),
                'up_wh_1': tf.Variable(tf.random_uniform([2 * self.hidden_size], -0.01, 0.01)),
                'up_wh_2': tf.Variable(tf.random_uniform([2 * self.hidden_size], -0.01, 0.01)),
    ...
    def user_product_attention(self):
        inputs = tf.reshape(self.x, [-1, self.max_sen_len, self.embedding_dim])
        sen_len = tf.reshape(self.sen_len, [-1])
        with tf.name_scope('up_word_encode'):
            outputs, state = tf.nn.bidirectional_dynamic_rnn(
                cell_fw=tf.nn.rnn_cell.LSTMCell(self.hidden_size, forget_bias=1.0),
                cell_bw=tf.nn.rnn_cell.LSTMCell(self.hidden_size, forget_bias=1.0),
                inputs=inputs,
        ...
        batch_size = tf.shape(outputs)[0]
        with tf.name_scope('up_word_attention'):
            output = tf.reshape(outputs, [-1, 2 * self.hidden_size])
            u = tf.matmul(output, self.weights['up_wh_1']) + self.biases['up_wh_1']
            u = tf.reshape(u, [-1, self.max_doc_len*self.max_sen_len, 2*self.hidden_size])
            u += tf.matmul(self.user, self.weights['wupu_1'])[:,None,:]
            u += tf.matmul(self.product, self.weights['wupp_1'])[:,None,:]
            u = tf.tanh(u)
            u = tf.reshape(u, [-1, 2 * self.hidden_size])
            alpha = tf.reshape(tf.matmul(u, self.weights['up_v_1']),
                               [batch_size, 1, self.max_sen_len])
            alpha = self.softmax(alpha, self.sen_len, self.max_sen_len)
            outputs = tf.matmul(alpha, outputs)
        ...
        outputs = tf.reshape(outputs, [-1, self.max_doc_len, 2 * self.hidden_size])
        with tf.name_scope('up_sentence_encode'):
            outputs, state = tf.nn.bidirectional_dynamic_rnn(
                cell_fw=tf.nn.rnn_cell.LSTMCell(self.hidden_size, forget_bias=1.0),
                cell_bw=tf.nn.rnn_cell.LSTMCell(self.hidden_size, forget_bias=1.0),
                inputs=outputs,
        ...
        batch_size = tf.shape(outputs)[0]
        with tf.name_scope('up_sentence_attention'):
            output = tf.reshape(outputs, [-1, 2 * self.hidden_size])
            u = tf.matmul(output, self.weights['up_wh_2']) + self.biases['up_wh_2']
            u = tf.reshape(u, [-1, self.max_doc_len, 2*self.hidden_size])
            u += tf.matmul(self.user, self.weights['wupu_2'])[:,None,:]
            u += tf.matmul(self.product, self.weights['wupp_2'])[:,None,:]
            u = tf.tanh(u)
            u = tf.reshape(u, [-1, 2*self.hidden_size])
            alpha = tf.reshape(tf.matmul(u, self.weights['up_v_2']),
                               [batch_size, 1, self.max_doc_len])
            alpha = self.softmax(alpha, self.doc_len, self.max_doc_len)
            outputs = tf.matmul(alpha, outputs)

        with tf.name_scope('up_softmax'):
            self.up_doc = tf.reshape(outputs, [batch_size, 2 * self.hidden_size])
            self.up_scores = tf.matmul(self.up_doc, self.weights['up_softmax']) + self.biases['up_softmax']
            self.up_predictions = tf.argmax(self.up_scores, 1, name="up_predictions")

        with tf.name_scope("up_loss"):
            losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.up_scores, labels=self.input_y)
            self.up_loss = tf.reduce_mean(losses)

        with tf.name_scope("up_accuracy"):
            correct_predictions = tf.equal(self.up_predictions, tf.argmax(self.input_y, 1))
            self.up_correct_num = tf.reduce_sum(tf.cast(correct_predictions, dtype=tf.int32))
            self.up_accuracy = tf.reduce_mean(tf.cast(correct_predictions, "float"), name="up_accuracy")
    ...
    def build_model(self):
        ...
        self.user_product_attention()

        with tf.name_scope('softmax'):
            outputs = tf.concat([self.u_doc, self.p_doc, self.h_doc, self.t_doc, self.tp_doc, self.tu_doc, self.up_doc], 1)
            self.scores = tf.matmul(outputs, self.weights['softmax']) + self.biases['softmax']
            self.predictions = tf.argmax(self.scores, 1, name="predictions")

        with tf.name_scope("loss"):
            losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.scores, labels=self.input_y)
            self.loss = 0.3*tf.reduce_mean(losses) + (0.1*self.u_loss  + 0.1*self.p_loss + 0.1*self.h_loss + 0.1*self.tp_loss + 0.1*self.tu_loss + 0.1*self.t_loss + 0.1*self.up_loss)
