...
class huapahata(object):
    ...
    def __init__(self, max_sen_len, max_doc_len, class_num, embedding_file,
        ...
        with tf.name_scope('weights'):
            self.weights = {
                'softmax': tf.Variable(tf.random_uniform([2 * 8 * self.hidden_size, self.class_num], -0.01, 0.01)),
    ...
    def build_model(self):
        ...
        with tf.name_scope("loss"):
            ...
            self.loss = 0.8 * tf.reduce_mean(losses) \
                        + 0.1 * (0.6*tf.reduce_mean(losses_d) + 0.4*(0.25*self.d_u_loss  + 0.25*self.d_p_loss + 0.25*self.d_h_loss + 0.25*self.d_t_loss)) \
                        + 0.1 * (0.6*tf.reduce_mean(losses_s) + 0.4*(0.25*self.s_u_loss  + 0.25*self.s_p_loss + 0.25*self.s_h_loss + 0.25*self.s_t_loss))
