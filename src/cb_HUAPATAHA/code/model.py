...
class huapahata(object):
    ...
    def __init__(self, max_sen_len, max_doc_len, class_num, embedding_file,
        ...
        with tf.name_scope('weights'):
            self.weights = {
                'softmax': tf.Variable(tf.random_uniform([12 * self.hidden_size, self.class_num], -0.01, 0.01)),
    ...
    def build_model(self):
        ...
        with tf.name_scope('softmax'):
            outputs = tf.concat([self.u_doc, self.p_doc, self.h_doc, self.t_doc, self.tp_doc, self.tu_doc], 1)
            self.scores = tf.matmul(outputs, self.weights['softmax']) + self.biases['softmax']
            self.predictions = tf.argmax(self.scores, 1, name="predictions")

        with tf.name_scope("loss"):
            losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.scores, labels=self.input_y)
            self.loss = 0.4*tf.reduce_mean(losses) + 0.1*self.u_loss  + 0.1*self.p_loss + 0.1*self.h_loss + 0.1*self.tp_loss + 0.1*self.tu_loss + 0.1*self.t_loss
