...
class huapahata(object):
    ...
    def user_attention(self):
        ...
        inputs2 = tf.concat([inputs, inputs2], 2)
        rnn_inputs = inputs2 # tf.nn.dropout(inputs2, keep_prob=.5)
        ...
        inputs3 = tf.reshape(outputs2, [-1, self.max_sen_len, self.hidden_size*2])
        inputs3 = tf.concat([inputs2, inputs3], 2)
        ...
        sinputs2 = tf.reshape(outputs, [-1, self.max_doc_len, 2 * self.hidden_size])
        sinputs2 = tf.concat([sinputs1, outputs], 2)
